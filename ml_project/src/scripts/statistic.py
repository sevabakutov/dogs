...
def get_dfs(dist, start_date_train, end_date_train, start_date_pred, end_date_pred):
    file_path = os.path.join(DATASET_DIR, f"dataset_{dist}.0.csv")
...
def save_encoder(encoder, dist, start_date, end_date) -> None:
    ...
    filename = f"encoder_{dist}_{safe_start_date}_{safe_end_date}.pkl"
...
def save_imputer(imputer, dist, start_date, end_date):
    safe_start_date = start_date.replace("/", "_")
    safe_end_date = end_date.replace("/", "_")
    filename = f"imputer_{dist}_{safe_start_date}_{safe_end_date}.pkl"
    filepath = os.path.join(IMPUTERS_DIR, filename)
    filepath = filepath.replace("\\", "/")
...
def save_model(model, dist, start_date, end_date):
    safe_start_date = start_date.replace("/", "_")
    safe_end_date = end_date.replace("/", "_")
    filename = f"model_{dist}_{safe_start_date}_{safe_end_date}.pkl"
...
def train_model(df_full, *args):
    ...
    logger.debug("Saving trained imputer")
    save_imputer(imputer, *args)

    logger.debug("Transformig dataset into numpy array...")
